...
from Models import User_model, User_message_model
import uvicorn
...
async def generate_answer(user_message:User_message_model,
    ...
    if not is_active_session:
        await chat_session_repo.delete_chat_session(user_message.session_id)
        raise HTTPException(
            status_code=404,
    ...
    generated_answer = 'hello from backend !'
...
async def fetch_history(jwtTokenPayload: dict = Depends(endpoint_verification),
    ...
    if not active_session:
        await chat_session_repo.delete_all_sessions_for_user(user_id)
        raise HTTPException(
            status_code=404,
