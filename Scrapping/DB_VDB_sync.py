...
def delete_duplicates_vdb(vdb:Vector_DB_interface):
    ...
    qdrant_id_urls = {item['qdrant_id']: item['source_url'] for item in all_items}
    url_counts = Counter(all_urls)
    deleted = 0
    for qdrant_id, url in qdrant_id_urls.items():
        if url_counts[url] > 1:
            vdb.delete_by_qdrant_id(qdrant_id)
            url_counts[url] -= 1
            deleted += 1
...
def sync(db:Rent_offers_repository,
    ...
    vdb_items = vdb.get_all_metadata()
    vdb_postgres_to_qdrant = {i['id']: i['qdrant_id'] for i in vdb_items if 'id' in i}
    vdb_postgres_ids = set(vdb_postgres_to_qdrant.keys())

    db_not_vdb = db_ids - vdb_postgres_ids
    vdb_not_db = vdb_postgres_ids - db_ids
    ...
    if len(vdb_not_db) > 0:
        for k in vdb_not_db:
            vdb.delete_by_qdrant_id(vdb_postgres_to_qdrant[k])
