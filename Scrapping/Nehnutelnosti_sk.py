...
class Nehnutelnosti_sk_processor:
    ...
    def get_key_attributes(self, soup):
        icons = {
            "rooms": "M16 4v6h4V6a2 2 0 0 0-2-2zm4 8h-4a2 2 0 0 1-2-2V4H6a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2zM2 6a4 4 0 0 1 4-4h12a4 4 0 0 1 4 4v12a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4z",
    ...
    def get_price(self,soup):
        ...
        price_energies = soup.find('p', "MuiTypography-root MuiTypography-label2 mui-gsg6ma"
                               ).text.strip()
        price_ms = soup.find('p', "MuiTypography-root MuiTypography-label2 mui-ifbhxp"
                                   ).text.strip()

        prices["rent"] = re.search(r"(.+?)\s?€", price_rent
                                   ).group(1)
        try:
            prices["energies"] = re.search(r"(.+?)\s?€", price_energies
                                           ).group(1)
        except:
            prices["energies"] = None
        try:
            prices["meter squared"] = re.search(r"(.+?)\s?€", price_ms
                                                ).group(1)
        except:
